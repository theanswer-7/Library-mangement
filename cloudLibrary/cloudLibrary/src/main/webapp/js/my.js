...
function renewBook(bid) {
    var r = confirm("确定续借该图书5天?");
    if (r) {
        var url = getProjectPath() + "/book/renewBook?id=" + bid;
        $.get(url, function (response) {
            alert(response.message)
            if (response.success == true) {
                window.location.href = getProjectPath() + "/book/searchBorrowed";
...
function findUserById(uid) {
    ...
    $.get(url, function (response) {
        ...
        $("#pw").val(response.data.password);
        $("#urole").val(response.data.role);
        $("#uemail").val(response.data.email);
        $("#uhire").val(response.data.hiredate);
...
function reserveBook(bookId) {
    if (!confirm("确认预约该书？图书可借阅时将会通知您。")) return;
    var url = getProjectPath() + "/yuyue/reserve";
    $.post(url, {bookId: bookId}, function (response) {
        alert(response.message);
    }).fail(function() {
        alert("预约失败，请稍后重试");
    });
...
function removeUserById(uid) {
    var r = confirm("确定删除工号：" + uid + "的人员?");
    if (r) {
        var url = getProjectPath() + "/user/removeUserById?id=" + uid;
        $.get(url, function (response) {
            alert(response.message)
            if (response.success == true) {
                window.location.href = getProjectPath() + "/user/search";
...
//获取当前项目的名称（由JSP中定义的contextPath全局变量提供）
...
//数据展示页面分页插件的页码发送变化时执行
    function changePage(pageNo, pageSize) {
        ...
        var $form = $("<form>").attr({
            action: pageargs.gourl,
            method: "post",
            style: "display:none"
        });
        $form.append($("<input>").attr({type: "hidden", name: "pageNum", value: pageargs.cur}));
        $form.append($("<input>").attr({type: "hidden", name: "pageSize", value: pageargs.pagesize}));
        //如果跳转的是图书信息查询的相关链接，提交图书查询栏中的参数
        if (pageargs.gourl.indexOf("book") >= 0) {
            $form.append($("<input>").attr({type: "hidden", name: "name", value: bookVO.name}));
            $form.append($("<input>").attr({type: "hidden", name: "author", value: bookVO.author}));
            $form.append($("<input>").attr({type: "hidden", name: "press", value: bookVO.press}));
        }
        //如果跳转的是用户管理查询的相关链接，提交用户查询栏中的参数
        if (pageargs.gourl.indexOf("user") >= 0) {
            $form.append($("<input>").attr({type: "hidden", name: "id", value: userVO.id}));
            $form.append($("<input>").attr({type: "hidden", name: "name", value: userVO.name}));
        }
        //如果跳转的是借阅记录查询的相关链接，提交借阅记录查询栏中的参数
        if (pageargs.gourl.indexOf("record") >= 0) {
            $form.append($("<input>").attr({type: "hidden", name: "bookname", value: recordVO.bookname}));
            $form.append($("<input>").attr({type: "hidden", name: "borrower", value: recordVO.borrower}));
        }
        $("body").append($form);
        $form.submit();
    }
