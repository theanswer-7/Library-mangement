...
<script>
    ...
    $(function (){
        ...
        $('.userTr').each(function() {
            // 遍历所有的tr元素
            var username = $(this).find('td:eq(1)').text().trim();
            // 找到每行的第二个td，获取其文本并去除前后空格
            if (username === name) {
                // 如果用户名等于admin
        ...
        <div class="box-tools pull-right">
            <div class="has-feedback">
                <form action="${pageContext.request.contextPath}/user/search" method="post">
                    <input type="hidden" id="userName" value="${USER_SESSION.name}">
                    工号：<input name="id" value="${search.id}">&nbsp&nbsp&nbsp&nbsp
            ...
            <tr class="userTr">
                ...
                <td>${user.hiredate}</td>
                    ...
                    <c:if test="${user.status == 0}">
                        <td bgcolor="#adffa3">在职</td>
                    </c:if>
                    <c:if test="${user.status == 1}">
                        <td bgcolor="red">已离职</td>
                    </c:if>
                ...
                <td class="text-center">
                    <c:if test="${USER_SESSION.role =='ADMIN'}">
                        ...
                        <c:if test="${user.status == 1}">
                            <button type="button" class="btn bg-olive btn-xs" onclick="removeUserById(${user.id})">删除
                            </button>

                        </c:if>
                    </c:if>
                    <c:if test="${USER_SESSION.role !='ADMIN'}">
                        <c:if test="${USER_SESSION.name==user.name}">
                            <button type="button" class="btn bg-olive btn-xs" data-toggle="modal" data-target="#editModal"
                                    onclick="findUserById(${user.id})">修改
                            </button>
                        </c:if>
                ...
                <form id="editUser">
                    <table class="table table-bordered table-striped" width="800px">
                        ...
                        <tr>
							...
							<td>
                                <c:if test="${USER_SESSION.role !='ADMIN'}">
								<select class="form-control" disabled id="urole" name="role" >
									<option value="USER">普通员工</option>
									<option value="ADMIN">管理员</option>
								</select>
                                </c:if>
